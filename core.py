    ...
    def handle_super(self, prepared, on_air):
        try:
            if prepared._index == on_air._index:
                print('Index match.')
                if prepared.is_super == True and on_air.is_super != True:
                    ...
                    try:
                        ...
                        if 'shots' in json_data:
                            ...
                            if current_data != self.previous_data:
                                ...
                                self.parent.olv_preview.SetObjects(self.prepared_olv)
                                self.parent.olv_program.SetObjects(self.on_air_olv)
                                if (prepared.is_super and not on_air.is_super) and (self.prepared_olv[len(self.prepared_olv)+1].auto_played != True):
                                    keyboard.press_and_release(self.advance_key)
                                    print('Playing super.')
                                    prepared.auto_played = True
